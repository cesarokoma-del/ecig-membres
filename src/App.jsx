...
// ============================================================
// FIREBASE CONFIG (même projet que le logiciel admin)
// ============================================================
...
const styles = `
  ...
  * { margin:0; padding:0; box-sizing:border-box; -webkit-tap-highlight-color:transparent; }
  :root {
    ...
    --border: #e5e7eb;
    --radius: 16px;
    --shadow: 0 2px 16px rgba(27,107,58,0.10);
  }
  ...
  /* LOGIN */
  .login-bg {
    min-height:100vh; background:linear-gradient(160deg,var(--green-dark) 0%,var(--green) 60%,#2d8a52 100%);
  ...
  .login-select { width:100%; padding:14px 16px; border-radius:12px; border:2px solid var(--border); font-family:'Nunito',sans-serif; font-size:15px; color:var(--text); background:var(--bg); margin-bottom:16px; outline:none; appearance:none; -webkit-appearance:none; }
  ...
  .nav-item.active .nav-icon { filter:drop-shadow(0 0 4px rgba(27,107,58,0.5)); }
...
// ============================================================
function Spinner() {
  return <div className="loading"><div className="spinner"/><p className="loading-text">Chargement...</p></div>;
}
...
function BottomNav({ screen, setScreen }) {
  const tabs = [
    ...
    { id:"enseignements", icon:"📖", label:"Enseignements" },
...
function TopBar({ membre, onLogout }) {
  const initials = membre ? membre.prenom[0]+membre.nom[0] : "?";
  return (
    <div className="topbar">
      <div className="topbar-logo">
        <div className="topbar-icon" style={{padding:'2px'}}>   <img src="/LogoECIG.jpg" style={{width:'28px',height:'28px',objectFit:'contain',borderRadius:'50%'}} /> </div>
        <div>
    ...
    <div className="content">
      ...
      <div className="stats-row">
        <div className="stat-card">
          <div className="stat-amount">{totalCDF.toLocaleString()}</div>
          <div className="stat-devise">CDF</div>
          <div className="stat-label">Francs</div>
        </div>
        <div className="stat-card">
          <div className="stat-amount">{totalUSD.toLocaleString()}</div>
          <div className="stat-devise">USD</div>
          <div className="stat-label">Dollars</div>
        </div>
        <div className="stat-card">
          <div className="stat-amount">{totalEUR.toLocaleString()}</div>
          <div className="stat-devise">EUR</div>
          <div className="stat-label">Euros</div>
        </div>
      </div>

      <div className="card">
        <div className="card-header">
          <span className="card-title">Mes infos</span>
          <span className="card-icon">👤</span>
        </div>
        <div className="list-item" style={{borderBottom:'none',padding:'4px 0'}}>
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'8px',width:'100%'}}>
            <div><div className="form-label">Rôle</div><div style={{fontSize:'14px',fontWeight:'600'}}>{membre.role||"—"}</div></div>
...
function CalendrierScreen({ calendrier }) {
  ...
  const upcoming = [...calendrier].sort((a,b) => new Date(a.date) - new Date(b.date))
    .filter(e => e.date >= today);
  const past = [...calendrier].sort((a,b) => new Date(b.date) - new Date(a.date))
    .filter(e => e.date < today).slice(0,5);

  return (
    <div className="content">
      <div className="page-title">📅 Calendrier</div>
      <div className="page-sub">Événements de l'ECIG</div>

      <div className="section-title">🔜 À venir</div>
      <div className="card">
        {upcoming.length === 0 ? (
          <div className="empty"><div className="empty-icon">📅</div><div className="empty-text">Aucun événement à venir</div></div>
        ) : upcoming.map((e,i) => (
          <div className="list-item" key={i}>
            <div className="list-bullet gold">📅</div>
            <div className="list-body">
              <div className="list-title">{e.titre||e.title||e.nom}</div>
              <div className="list-desc">{e.description||e.lieu||""}</div>
              <div className="list-date">{e.date} {e.heure ? `• ${e.heure}` : ""}</div>
            </div>
          </div>
        ))}
      </div>
        ...
        <div className="card">
          {past.map((e,i) => (
            <div className="list-item" key={i} style={{opacity:0.6}}>
              ...
              <div className="list-body">
                <div className="list-title">{e.titre||e.title||e.nom}</div>
                <div className="list-date">{e.date}</div>
              </div>
            </div>
          ))}
        </div>
...
function AnnoncesScreen({ annonces }) {
  return (
    <div className="content">
      ...
      <div className="card">
        {annonces.length === 0 ? (
          <div className="empty"><div className="empty-icon">📢</div><div className="empty-text">Aucune annonce pour l'instant</div></div>
        ) : [...annonces].reverse().map((a,i) => (
          <div className="list-item" key={i}>
            <div className="list-bullet gold">📢</div>
            <div className="list-body">
              <div className="list-title">{a.titre||a.title||a.sujet}</div>
...
function EnseignementsScreen({ enseignements }) {
  ...
  if (selected) return (
    <div className="content">
      ...
      <div style={{fontSize:'12px',color:'var(--text-muted)',marginBottom:'16px'}}>{selected.date} • {selected.orateur||selected.pasteur||""}</div>
      <div className="card">
    ...
    <div className="content">
      ...
      <div className="card">
        {enseignements.length === 0 ? (
          <div className="empty"><div className="empty-icon">📖</div><div className="empty-text">Aucun enseignement disponible</div></div>
...
// ÉCRAN: COTISATIONS
// ============================================================
function CotisationsScreen({ membre, cotisations }) {
  const myCotis = cotisations
    .filter(c => c.membreId == membre.id)
  ...
  const typeEmoji = { dime:"🔟", offrande:"🙌", don:"🎁", autre:"💝" };

  return (
    <div className="content">
      <div className="page-title">💰 Mes cotisations</div>
      <div className="page-sub">{myCotis.length} opération(s)</div>
      ...
      <div className="card">
        {myCotis.length === 0 ? (
          <div className="empty"><div className="empty-icon">💰</div><div className="empty-text">Aucune cotisation enregistrée</div></div>
        ) : myCotis.map((c,i) => (
          <div className="list-item" key={i}>
            <div className="list-bullet gold">{typeEmoji[c.type]||"💝"}</div>
            <div className="list-body">
              <div className="list-title" style={{textTransform:'capitalize'}}>{c.type||"Cotisation"}</div>
              {c.note && <div className="list-desc">{c.note}</div>}
              <div className="list-date">{c.date}</div>
            </div>
            <div style={{textAlign:'right',flexShrink:0}}>
              <div style={{fontWeight:'800',fontSize:'15px',color:'var(--green)'}}>{Number(c.montant).toLocaleString()}</div>
              <span className="badge badge-gold" style={{marginTop:'2px'}}>{c.devise||"CDF"}</span>
            </div>
          </div>
        ))}
      </div>
...
function ProfilScreen({ membre, membres, setMembres }) {
  ...
  return (
    <div className="content">
      ...
      <div className="profile-role">{membre.role} • {membre.departement}</div>

      <div className="card">
        <div className="section-title">✏️ Modifier mon profil</div>
        <div className="form-group">
          <div className="form-label">Téléphone</div>
          <input className="form-input" value={form.telephone||""} onChange={e=>setForm({...form,telephone:e.target.value})} placeholder="+243..." />
        </div>
        <div className="form-group">
        ...
        <div className="form-group">
          ...
          <input className="form-input" value={form.adresse||""} onChange={e=>setForm({...form,adresse:e.target.value})} placeholder="Adresse..." />
        </div>
      ...
      <div className="pin-section">
        ...
        <div className="form-group">
          <div className="form-label">Ancien PIN</div>
          <input className="form-input" type="password" maxLength={4} value={pinForm.old} onChange={e=>setPinForm({...pinForm,old:e.target.value})} placeholder="••••" />
        </div>
        <div className="form-group">
          <div className="form-label">Nouveau PIN (4 chiffres)</div>
          <input className="form-input" type="password" maxLength={4} value={pinForm.nouveau} onChange={e=>setPinForm({...pinForm,nouveau:e.target.value})} placeholder="••••" />
        </div>
        <div className="form-group">
          <div className="form-label">Confirmer nouveau PIN</div>
          <input className="form-input" type="password" maxLength={4} value={pinForm.confirm} onChange={e=>setPinForm({...pinForm,confirm:e.target.value})} placeholder="••••" />
        </div>
...
export default function App() {
  ...
  const handleLogin = async () => {
    ...
    if (pinStr !== correctPin) { setError("PIN incorrect. Essayez encore."); setPin(["","","",""]); return; }
    setLoading(true);
    const [cotis, anno, cal, ens] = await Promise.all([
      loadData("ecig_cotisations", []),
      loadData("ecig_annonces", []),
      loadData("ecig_calendrier", []),
  ...
  <img src="/LogoECIG.png" style={{width:'60px',height:'60px',objectFit:'contain'}} />
        ...
        <div className="login-card">
          ...
          {loadingMembres ? (
            <div style={{textAlign:'center',padding:'20px',color:'var(--text-muted)'}}>Chargement des membres...</div>
          ) : <>
            <select className="login-select" value={selectedId} onChange={e=>{setSelectedId(e.target.value);setError("");}}>
              ...
              {membres.map(m=>(
                <option key={m.id} value={m.id}>{m.prenom} {m.nom} {m.role ? `(${m.role})` : ""}</option>
              ))}
            </select>
  ...
  return (
    <div className="app">
      <style>{styles}</style>
      <TopBar membre={membre} onLogout={handleLogout} />
      {screen==="home" && <HomeScreen membre={membre} cotisations={cotisations}/>}
      ...
      {screen==="cotisations" && <CotisationsScreen membre={membre} cotisations={cotisations}/>}
      {screen==="profil" && <ProfilScreen membre={membre} membres={membres} setMembres={setMembres}/>}
